...
\documentclass[master,oneside,AutoFakeBold,nocpsupervisor]{zjuthesis} % 如果你的论文不满80页，还是单面印刷吧

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 开始填写前置部分使用的变量
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 样式设定在 zjuthesis.cls 下, 人类可读，爱请查阅

% 这里写这么鬼畜是为了测试多几个字会不会造成溢出
\title{基于热点感知的资源受限环境下操作系统快照优化研究} % 封面和题名页使用
\englishtitle{Optimization of Operating System Snapshots in Resource-Constrained Environments Based on Hotspot Perception} % 封面和题名页使用
...
\secretlevel{无}            % 封面头使用
\studentnumber{22351067}    % 封面头使用

%\supervisor{御坂美琴}       % 导师 封面使用
\supervisor{赵新奎}
%\spvtitle{电击使}           % 职称 封面使用
\spvtitle{百人计划研究员}              % 职称 封面使用
...
\submitdate{2025年12月18日}   % 论文提交日期 栏
...
\include{contents/hotspot}
\include{contents/strategy}
\include{contents/flash}
\include{contents/eval}
